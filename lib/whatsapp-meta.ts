...
// Get WhatsApp Group ID from env
...
export async function sendWhatsAppMessage(
  ...
  try {
    // Ensure phone number/group ID format (remove + and spaces)
    const cleanRecipient = message.to.replace(/[\s+]/g, "");
...
export async function sendToGroup(
  ...
  if (!groupId) {
    return {
      success: false,
      error: "WhatsApp group ID not configured in environment variables",
    };
  }
...
export async function sendCustomReminder(
  ...
  return sendToGroup(formattedMessage);
}
